...
public class GitHelper {
	...
	public static PairBeforeAfter<SourceFileSet> getSourcesBeforeAndAfterCommit(GitRepository repo, String commitSha1) throws VcsException {
		try {
			...
			for (Change change : changes) {
				...
				if (beforeRevision != null) {
					SourceFile current = new SourceFile(Paths.get(beforeRevision.getFile().getPath()));
					filesBefore.add(current);
					mapBefore.put(current, beforeRevision);
				}
				if (afterRevision != null) {
					SourceFile current = new SourceFile(Paths.get(afterRevision.getFile().getPath()));
					filesAfter.add(current);
					mapAfter.put(current, afterRevision);
				}
			}
